...
public class LinearTouchPhidgetManager extends Manager implements
	...
	/**
	 * Creates a new LinearTouchPhidgetManager and connects to VLC's telnet
	 * interface. Once connected to VLC, attach and detach listeners are added
	 * which monitor when a phidiget is plugged in or removed from the computer.
	 * 
	...
	public void detached(DetachEvent event) {
		try {
			String message = "Removing " + event.getSource().getDeviceName()
					+ " (" + event.getSource().getSerialNumber() + ")";
	...
	public void attached(AttachEvent event) {
		...
		try {
			String message = "Attached new Phidget\n"
					+ event.getSource().getDeviceName() + " "
					+ event.getSource().getDeviceLabel() + " "
					+ event.getSource().getDeviceType() + " "
					+ event.getSource().getSerialNumber();
			Log.log(Level.INFO, message);
